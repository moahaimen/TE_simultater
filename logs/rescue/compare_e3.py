"""Compare Check #3 rescue bundle against Task A baseline.

Usage:
    python logs/rescue/compare_e3.py
...
per the pre-registered verdict rule in SESSION_HANDOFF.
"""
...
RESCUE = ROOT / "results" / "rescue_e3_unseen_dnh_1p02" / "packet_sdn_summary.csv"
...
def main() -> int:
    ...
    seen_mlu_rel = seen_rows["mean_mlu__rel_pct"].abs().max() if len(seen_rows) else 0.0
    unseen_mlu_rel = unseen_rows["mean_mlu__rel_pct"].abs().max() if len(unseen_rows) else 0.0
    unseen_dt_mean_rel = unseen_rows["decision_time_ms__rel_pct"].mean() if len(unseen_rows) else 0.0

    print("\n" + "=" * 80)
    print("VERDICT INPUTS")
    print("=" * 80)
    print(f"  max |seen MLU rel%|   = {seen_mlu_rel:+.3f}%   (guardrail: <=1.0%)")
    print(f"  max |unseen MLU rel%| = {unseen_mlu_rel:+.3f}%   (guardrail: <=1.0%)")
    print(f"  mean unseen dt rel%   = {unseen_dt_mean_rel:+.3f}%   (target:   <=-5.0%)")
    ...
    elif seen_mlu_rel > 1.0:
        verdict = "FAIL"
        reason = f"Seen MLU moved {seen_mlu_rel:.2f}% (>1% guardrail)."
    elif unseen_mlu_rel > 1.0:
        verdict = "FAIL"
        reason = f"Unseen MLU moved {unseen_mlu_rel:.2f}% (>1% guardrail)."
    elif unseen_dt_mean_rel <= -5.0:
        verdict = "PASS"
        reason = f"Unseen decision time ↓{-unseen_dt_mean_rel:.2f}% with MLU guardrails held."
    else:
        verdict = "FAIL"
        reason = f"Unseen decision time moved {unseen_dt_mean_rel:+.2f}% (<5% improvement target)."
    print(f"VERDICT: {verdict}")
    print(f"REASON : {reason}")
    print("=" * 80)

    out.to_csv(ROOT / "results" / "rescue_e3_unseen_dnh_1p02" / "delta_table.csv", index=False)
