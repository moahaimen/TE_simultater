...
def build_graph_tensors_plus(
    ...
    if path_library is not None and tm_vector is not None:
        ...
        for od_idx in range(len(path_library.edge_idx_paths_by_od)):
            if tm[od_idx] < 1e-12:
                continue
            for path_edges in path_library.edge_idx_paths_by_od[od_idx]:
        ...
        for od_idx in range(len(path_library.edge_idx_paths_by_od)):
            ...
            if costs:
                ...
                if best_path_idx < len(edge_paths) and edge_paths[best_path_idx]:
                    ...
                    if path_edges:
                        path_utils = [util[e] for e in path_edges if e < num_edges]
                        if path_utils:
                            max_u = max(path_utils)
                            for e in path_edges:
                                if e < num_edges and util[e] >= max_u - 1e-12:
                                    is_bottleneck[e] = 1.0
    ...
    if tm_vector is not None:
        ...
        for od_idx, (s, d) in enumerate(od_pairs):
            si, di = node_to_idx[s], node_to_idx[d]
            demand_through_node[si] += tm[od_idx]
            demand_through_node[di] += tm[od_idx]
    ...
    for i in range(num_nodes):
        ...
        for nb in neighbors[i]:
            if max_util_in[nb] > 0.8 or max_util_out[nb] > 0.8:
                congested_count += 1
        congested_neighbor_frac[i] = congested_count / len(neighbors[i])

    # 14: max_residual_capacity on incident edges
    max_residual_cap = np.zeros(num_nodes, dtype=np.float64)
    for eidx, (u, v) in enumerate(edges):
        ui, vi = node_to_idx[u], node_to_idx[v]
        rc = residual_cap[eidx]
        max_residual_cap[ui] = max(max_residual_cap[ui], rc)
        max_residual_cap[vi] = max(max_residual_cap[vi], rc)
    ...
    for i in range(num_nodes):
        ...
        for a_idx in range(k):
            for b_idx in range(a_idx + 1, k):
                if nbrs[b_idx] in neighbors[nbrs[a_idx]]:
                    triangles += 1
...
def build_od_features_plus(
    ...
    best_path_edges_list = [[] for _ in range(num_od)]  # for overlap computation
    ...
    max_util_in = np.zeros(num_nodes, dtype=np.float64)
    max_util_out = np.zeros(num_nodes, dtype=np.float64)
    for eidx, (u, v) in enumerate(dataset.edges):
        ui, vi = node_to_idx[u], node_to_idx[v]
        max_util_out[ui] = max(max_util_out[ui], util[eidx])
        max_util_in[vi] = max(max_util_in[vi], util[eidx])
    ...
    for od_idx in range(num_od):
        ...
        if costs:
            ...
            sensitivity_scores[od_idx] = float(tm[od_idx]) * float(min_cost)
            num_paths[od_idx] = len(costs)

            best_path_idx = int(np.argmin(costs))

            # Hop count from best path
            if best_path_idx < len(edge_paths) and edge_paths[best_path_idx]:
                path_edges = edge_paths[best_path_idx]
                hop_count[od_idx] = len(path_edges)
                best_path_edges_list[od_idx] = list(path_edges)
            ...
            # Alternative path headroom: best headroom among non-best paths
            for pidx, ep in enumerate(edge_paths):
                if pidx == best_path_idx or not ep:
                    continue
                path_utils = [util[e] for e in ep if e < num_edges]
                if path_utils:
                    path_bottleneck = max(path_utils)
                    path_headroom = 1.0 - path_bottleneck
                    alt_path_headroom[od_idx] = max(alt_path_headroom[od_idx], path_headroom)
    ...
    edge_od_count = np.zeros(num_edges, dtype=np.float64)
    for od_idx in range(num_od):
        if tm[od_idx] < 1e-12:
            continue
        for eidx in best_path_edges_list[od_idx]:
            if eidx < num_edges:
                edge_od_count[eidx] += 1

    path_overlap = np.zeros(num_od, dtype=np.float64)
    for od_idx in range(num_od):
        edges_on_path = best_path_edges_list[od_idx]
        if not edges_on_path:
            continue
        shared_count = sum(1 for e in edges_on_path if e < num_edges and edge_od_count[e] > 1)
        path_overlap[od_idx] = shared_count / len(edges_on_path)
